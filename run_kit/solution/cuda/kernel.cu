...
__device__ inline void run_topk_warps_sorted_merge(
    ...
    // stub: fill out_topk_indices with -1
    const int tid = topk_warp * 32 + lane;
    const int team_threads = kNumTopkWarps * 32;
    for (int i = tid; i < topk_count; i += team_threads) {
        out_topk_b[i] = -1;
    }
}
...
__global__ __launch_bounds__(kThreadsPerBlock) void dsa_topk_indexer_kernel(
    ...
    int* out_topk_indices,          // [B,topk_count]
    int batch_size,
    int num_pages,
    int max_num_pages,
    int out_stride,
