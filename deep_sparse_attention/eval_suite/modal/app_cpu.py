...
def run_eval(submission_code: str, tests_content: str, mode: str = "test", workspace_name: str = "sparse_attention", swap_ok: bool = False) -> dict:
    ...
    proc = subprocess.Popen(
        [_sys.executable, eval_script, mode, "tests.txt", "--no-compile", "--prebuilt-so", EXT_CACHE_LOCAL] + (["--swap_ok"] if swap_ok else []),
...
def _format_benchmarks(benchmarks: dict) -> list[str]:
    ...
    for idx, info in items:
        ...
        check_msg = info.get("check")
        if check_msg:
            for cl in check_msg.split("\\n"):
                lines.append(f" {cl}")
...
def main():
    ...
    parser.add_argument("--swap_ok", action="store_true", help="Allow swapped positions in top-k to pass")
    ...
    with app.run():
        ...
        # Step 2: GPU eval
        result = run_eval.remote(submission_code, tests_content, args.mode, workspace, swap_ok=args.swap_ok)
