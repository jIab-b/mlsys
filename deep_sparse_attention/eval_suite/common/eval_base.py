...
class EvalRunner:
    ...
    swap_ok = False  # Set via --swap_ok CLI flag
    ...
    def run_single_benchmark(
        ...
        return BenchmarkResult(stats=calculate_stats(durations), error=correctness_error, message=correctness_message)
    ...
    def run_single_benchmark_batched(
        ...
        for ref_output, cust_output in zip(check_copy, outputs):
            ...
            if correctness_message is None:
                correctness_message = message
            if not good:
                correctness_error = message
                break
...
def run_testing(logger: PopcornOutput, pool: multiprocessing.Pool, tests: list[TestCase], runner: EvalRunner):
    ...
    for idx, test in enumerate(tests):
        ...
        if not good:
            ...
            logger.log(f"test.{idx}.error", message.replace("\n", "\\n") if message else "")
            passed = False
        else:
            logger.log(f"test.{idx}.status", "pass")
            if message:
                logger.log(f"test.{idx}.message", message.replace("\n", "\\n"))
...
def run_benchmarking(logger: PopcornOutput, pool: multiprocessing.Pool, tests: list[TestCase], runner: EvalRunner):
    ...
    for idx, test in enumerate(tests):
        ...
        if result.message is not None:
            logger.log(f"benchmark.{idx}.check", result.message.replace("\n", "\\n"))
