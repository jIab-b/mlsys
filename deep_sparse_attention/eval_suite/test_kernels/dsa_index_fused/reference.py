"""DSA top-k=2048 reference – scale after MMA (matches ld_smem computation order)."""
...
def check_implementation(data, output, swap_ok=False):
    ...
    pos_mismatches = int((got != ref).sum().item())
    exact_match = pos_mismatches == 0

    # --- set-based match (value recall) ---
    set_wrong = 0
    set_details = []
    for b in range(batch):
        ...
        r_set = set(r_valid.tolist())
        missing = r_set - g_set
        extra = g_set - r_set
        if missing or extra:
            set_wrong += len(missing)
            if len(set_details) < 3:
                set_details.append(f"  b={b}: missing={len(missing)} extra={len(extra)}")

    total_valid = sum(int((ref[b] >= 0).sum().item()) for b in range(batch))
    recall = (total_valid - set_wrong) / max(total_valid, 1)

    lines = []
    lines.append(f"exact: {pos_mismatches}/{batch * topk} positional mismatches")
    lines.append(f"set:   recall={recall:.4f} ({set_wrong}/{total_valid} values wrong)")
    if set_details:
        lines.extend(set_details)
