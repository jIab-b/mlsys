"""Debug version of DSA index submission with guarded printf at all potential error areas."""
...
// ---------------------------------------------------------------------------------
// Debug flag: only first thread of first CTA prints, once
// ---------------------------------------------------------------------------------
__device__ static int g_dbg_printed = 0;

#define DBG_GUARD (blockIdx.x == 0 && threadIdx.x == 0 && atomicCAS(&g_dbg_printed, 0, 1) == 0)
#define DBG_ONCE(fmt, ...) do { if (blockIdx.x == 0 && threadIdx.x == 0) { printf(fmt, ##__VA_ARGS__); } } while(0)

// Per-warp debug: lane 0 of the specific warp in CTA 0
#define DBG_WARP(wid, fmt, ...) do { \
    if (blockIdx.x == 0 && (threadIdx.x >> 5) == (wid) && (threadIdx.x & 31) == 0) { \
...
// ---------------------------------------------------------------------------------
// Kernel: 3-stage warp-specialized pipeline (DEBUG VERSION)
// ---------------------------------------------------------------------------------
__global__ void dsa_topk_indexer_kernel(
    ...
    // DBG: Print kernel launch params
    DBG_ONCE("[DBG] kernel entry: b=%d batch_size=%d num_pages=%d max_num_pages=%d topk=%d seq_len=%d\\n",
             b, batch_size, num_pages, max_num_pages, topk, seq_len);
    ...
    if (seq_len == 0 || topk <= 0) {
        DBG_ONCE("[DBG] early exit: seq_len=%d topk=%d\\n", seq_len, topk);
        return;
    }
    ...
    size_t smem_used = static_cast<size_t>(smem_ptr - smem_raw);

    // DBG: Print smem layout
    DBG_ONCE("[DBG] smem_raw=%p (generic), smem_shared_addr=0x%x\\n",
             smem_raw, cvta_to_shared_u32(smem_raw));
    DBG_ONCE("[DBG] smem alignment: raw%%1024=%lu\\n",
             (unsigned long)(reinterpret_cast<uintptr_t>(smem_raw) & 1023));
    DBG_ONCE("[DBG] smem total used: %lu bytes\\n", (unsigned long)smem_used);
    DBG_ONCE("[DBG] k_stage_payload: offset=%lu, shared=0x%x\\n",
             (unsigned long)(reinterpret_cast<unsigned char*>(k_stage_payload) - smem_raw),
             cvta_to_shared_u32(k_stage_payload));
    DBG_ONCE("[DBG] k_stage_scale: offset=%lu, shared=0x%x\\n",
             (unsigned long)(reinterpret_cast<unsigned char*>(k_stage_scale) - smem_raw),
             cvta_to_shared_u32(k_stage_scale));
    DBG_ONCE("[DBG] q_stage: offset=%lu, shared=0x%x\\n",
             (unsigned long)(reinterpret_cast<unsigned char*>(q_stage) - smem_raw),
             cvta_to_shared_u32(q_stage));
    DBG_ONCE("[DBG] w_stage: offset=%lu, shared=0x%x\\n",
             (unsigned long)(reinterpret_cast<unsigned char*>(w_stage) - smem_raw),
             cvta_to_shared_u32(w_stage));
    DBG_ONCE("[DBG] tma_mbar: offset=%lu, shared=0x%x, align%%8=%lu\\n",
             (unsigned long)(reinterpret_cast<unsigned char*>(tma_mbar) - smem_raw),
             cvta_to_shared_u32(tma_mbar),
             (unsigned long)(reinterpret_cast<uintptr_t>(tma_mbar) & 7));
    DBG_ONCE("[DBG] mma_mbar: offset=%lu, shared=0x%x, align%%8=%lu\\n",
             (unsigned long)(reinterpret_cast<unsigned char*>(mma_mbar) - smem_raw),
             cvta_to_shared_u32(mma_mbar),
             (unsigned long)(reinterpret_cast<uintptr_t>(mma_mbar) & 7));
    DBG_ONCE("[DBG] epi_mbar: offset=%lu, shared=0x%x, align%%8=%lu\\n",
             (unsigned long)(reinterpret_cast<unsigned char*>(epi_mbar) - smem_raw),
             cvta_to_shared_u32(epi_mbar),
             (unsigned long)(reinterpret_cast<uintptr_t>(epi_mbar) & 7));
    DBG_ONCE("[DBG] tmem_addr_scratch: offset=%lu, shared=0x%x\\n",
             (unsigned long)(reinterpret_cast<unsigned char*>(tmem_addr_scratch) - smem_raw),
             cvta_to_shared_u32(tmem_addr_scratch));
    DBG_ONCE("[DBG] topk_scores: offset=%lu, shared=0x%x\\n",
             (unsigned long)(reinterpret_cast<unsigned char*>(topk_scores) - smem_raw),
             cvta_to_shared_u32(topk_scores));
    DBG_ONCE("[DBG] topk_ids: offset=%lu, shared=0x%x\\n",
             (unsigned long)(reinterpret_cast<unsigned char*>(topk_ids) - smem_raw),
             cvta_to_shared_u32(topk_ids));

    // DBG: Check TMA dest alignment (must be 128-byte aligned for TMA)
    DBG_ONCE("[DBG] payload TMA dest align%%128: stage0=%lu\\n",
             (unsigned long)(reinterpret_cast<uintptr_t>(k_stage_payload) & 127));
    DBG_ONCE("[DBG] scale TMA dest align%%128: stage0=%lu (addr=0x%x)\\n",
             (unsigned long)(reinterpret_cast<uintptr_t>(k_stage_scale) & 127),
             cvta_to_shared_u32(k_stage_scale));
    ...
    DBG_ONCE("[DBG] mbarrier init done, fence done\\n");
    ...
    // DBG: Print TMEM alloc result
    DBG_ONCE("[DBG] tmem_addr_scratch[0] = %d (TMEM base addr)\\n", tmem_addr_scratch[0]);
    ...
    DBG_ONCE("[DBG] kIdesc = 0x%x (atype=E4M3, btype=E4M3, dtype=F32, N=%d>>3, M=%d>>4)\\n",
             kIdesc, kStageTokens, kNumHeads);

    const int num_tiles = (seq_len + kStageTokens - 1) / kStageTokens;
    DBG_ONCE("[DBG] num_tiles=%d, actual_topk=%d\\n", num_tiles, actual_topk);

    // DBG: Print global pointers
    DBG_ONCE("[DBG] k_fp8_tmap=%p, k_scale_tmap=%p\\n", k_fp8_tmap, k_scale_tmap);
    DBG_ONCE("[DBG] q_index_bytes=%p, k_index_cache=%p, weights=%p\\n",
             q_index_bytes, k_index_cache, weights);
            ...
            if (tile_id < num_tiles) {
                ...
                if (lane == 0) {
                    if (valid_tokens > 0) {
                        ...
                        // DBG: Print producer TMA details (first 2 tiles only)
                        if (tile_id < 2) {
                            printf("[DBG] producer tile=%d stage=%d page_idx=%d valid=%d\\n",
                                   tile_id, stage, page_idx, valid_tokens);
                            printf("[DBG]   payload_dst=0x%x (align%%128=%d)\\n",
                                   payload_dst, payload_dst & 127);
                            printf("[DBG]   scale_dst=0x%x (align%%128=%d)\\n",
                                   scale_dst, scale_dst & 127);
                            printf("[DBG]   mbar_addr=0x%x (align%%8=%d)\\n",
                                   mbar_addr, mbar_addr & 7);
                            printf("[DBG]   expect_tx=%d (payload=%d + scale=%d)\\n",
                                   payload_bytes + scale_bytes, payload_bytes, scale_bytes);
                            printf("[DBG]   tma3d coords: x=0 y=0 z=%d\\n", page_idx);
                            printf("[DBG]   tma2d coords: x=0 y=%d\\n", page_idx);
                        }
                        ...
                        // DBG
                        if (tile_id < 2) {
                            printf("[DBG] producer tile=%d stage=%d EMPTY (page_idx=%d valid=0)\\n",
                                   tile_id, stage, page_idx);
                        }
        ...
        if (warp_id == kMmaWarp) {
            ...
            if (tile_id >= 0 && tile_id < num_tiles) {
                ...
                if (valid_tokens > 0 && lane == 0) {
                    ...
                    // DBG: Print MMA details (first 2 tiles only)
                    if (tile_id < 2) {
                        printf("[DBG] MMA tile=%d stage=%d valid=%d\\n",
                               tile_id, stage, valid_tokens);
                        printf("[DBG]   q_addr=0x%x (align%%128=%d), k_addr=0x%x (align%%128=%d)\\n",
                               q_addr, q_addr & 127, k_addr, k_addr & 127);
                        printf("[DBG]   a_desc=0x%llx, b_desc=0x%llx\\n",
                               (unsigned long long)a_desc, (unsigned long long)b_desc);
                        printf("[DBG]   kIdesc=0x%x, tmem_d=0\\n", kIdesc);
                    }
                    ...
                    if (tile_id < 2) {
                        printf("[DBG] MMA tile=%d stage=%d SKIPPED (valid=0)\\n",
                               tile_id, stage);
                    }
        ...
        if (warp_id >= kEpilogueWarpBase && warp_id < kEpilogueWarpBase + kNumEpilogueWarps) {
            ...
            if (tile_id >= 0 && tile_id < num_tiles) {
                ...
                for (int t = 0; t < valid_tokens; ++t) {
                    ...
                    // DBG: Print epilogue values (first tile, first token only)
                    if (tile_id == 0 && t == 0 && lane == 0) {
                        printf("[DBG] epilogue tile=0 t=0 epi_warp=%d: d=%.6f scale=%.6f tmem_row=%d\\n",
                               epi_warp, d, scale, tmem_row_base);
                        printf("[DBG]   v=%.6f w=%.6f partial_sum=%.6f\\n",
                               v, w, my_sum);
                        printf("[DBG]   page_idx=%d token_id=%d\\n", page_idx, page_idx * kPageSize + t);
                    }
                ...
                if (epi_warp == 0 && lane == 0) {
                    ...
                    // DBG: Print topk insertion (first tile only)
                    if (tile_id == 0) {
                        printf("[DBG] epilogue tile=0 inserting %d candidates, topk_count_before=%d\\n",
                               valid_tokens, topk_count);
                    }
                    ...
                    if (tile_id == 0) {
                        printf("[DBG] epilogue tile=0 topk_count_after=%d\\n", topk_count);
                    }
    ...
    // DBG: Print final topk results
    DBG_ONCE("[DBG] pipeline done. topk_count=%d\\n", topk_count_ptr[0]);
    ...
    if (warp_id == kEpilogueWarpBase) {
        ...
        // DBG: Print first few output indices
        if (lane == 0 && blockIdx.x == 0) {
            printf("[DBG] output[0..3]: %d %d %d %d (topk_count=%d)\\n",
                   (actual_topk > 0) ? out_b[0] : -1,
                   (actual_topk > 1) ? out_b[1] : -1,
                   (actual_topk > 2) ? out_b[2] : -1,
                   (actual_topk > 3) ? out_b[3] : -1,
                   topk_count);
        }
    }
...
static void ensure_k_fp8_tmap_on_device(const int8_t* k_index_cache_ptr, int num_pages) {
    ...
    CUtensorMap k_scale_tmap{};
    {
        ...
        // DBG: Print tensor map setup on host side
        printf("[HOST DBG] scale TMA: base=%p, globalDim={%llu,%llu}, stride={%llu}, boxDim={%u,%u}\\n",
               scale_base,
               (unsigned long long)sglobalDim[0], (unsigned long long)sglobalDim[1],
               (unsigned long long)sglobalStrides[0],
               sboxDim[0], sboxDim[1]);
        printf("[HOST DBG] payload TMA: base=%p, globalDim={%llu,%llu,%llu}, stride={%llu,%llu}, boxDim={%u,%u,%u}\\n",
               k_index_cache_ptr,
               (unsigned long long)globalDim[0], (unsigned long long)globalDim[1], (unsigned long long)globalDim[2],
               (unsigned long long)globalStrides[0], (unsigned long long)globalStrides[1],
...
void dsa_topk_indexer_launch(
    ...
    printf("[HOST DBG] launch: batch=%d num_pages=%d max_num_pages=%d topk=%d\\n",
           batch_size, num_pages, max_num_pages, topk);
    printf("[HOST DBG] q shape: [%d, %ld, %ld], dtype=%d\\n",
           batch_size, (long)q_index_fp8.size(1), (long)q_index_fp8.size(2), (int)q_index_fp8.scalar_type());
    printf("[HOST DBG] k shape: [%d, %ld, %ld, %ld], dtype=%d\\n",
           num_pages, (long)k_index_cache_fp8.size(1), (long)k_index_cache_fp8.size(2), (long)k_index_cache_fp8.size(3),
           (int)k_index_cache_fp8.scalar_type());
    ...
    printf("[HOST DBG] smem: need=%lu, max_optin=%d, desired=%d\\n",
           (unsigned long)smem_need, max_optin_smem, kDesiredDynamicSmemBytes);
    ...
    printf("[HOST DBG] dynamic_smem_bytes=%d, threads=%d, blocks=%d\\n",
           dynamic_smem_bytes, kThreadsPerBlock, batch_size);
    ...
    // Reset debug flag before launch
    int zero = 0;
    cudaMemcpyToSymbol(g_dbg_printed, &zero, sizeof(int));
...
def _get_module():
    ...
    if _module is None:
        _module = load_inline(
            name="dsa_topk_indexer_debug_ext",
