...
constexpr int kKcDescK = kMmaK;  // atom32 experiment: keep KC descriptor K-span at 32
    ...
    if (warp_id == kScoreMmaWarp && lane == 0) {
        ...
        for (int local = 0; local < stages_total; ++local) {
            ...
            if (s.stage_valid[stage] > 0) {
                ...
                // Kc atom32 path: use MN-major SW128_32B descriptor tile [64,32].
                uint64_t kc_desc = make_umma_desc_from_smem<
                    cute::UMMA::Major::MN, cute::UMMA::Layout_MN_SW128_32B_Atom<__nv_bfloat16>,
                    kStageTokens, kKcDescK>(
        ...
        for (int local = 0; local < stages_total; ++local) {
            ...
            if (valid_stage) {
                ...
                for (int tile = 0; tile < kValMmaMTiles; ++tile) {
                    ...
                    // Kc value slice atom32 path: MN-major SW128_32B descriptor tile [128,32].
                    uint64_t kc_col_desc = make_umma_desc_from_smem<
                        cute::UMMA::Major::MN, cute::UMMA::Layout_MN_SW128_32B_Atom<__nv_bfloat16>,
                        kValMmaMTile, kKcDescK>(
                        addr.kc_stage + stage * kStageTokens * kCkvRowBytes +
