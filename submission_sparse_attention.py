"""Submission wrapper for sparse_attention kernel (fused index + attention).

This wraps both sparse_index.py and sparse_attn.py to match the eval_suite interface.
"""
import math
import torch
...
@torch.no_grad()
def _run_index(q_index_fp8, k_index_cache_fp8, weights, seq_lens, block_table):
...
# ============================================================================
# sparse_attn implementation
# ============================================================================

@torch.no_grad()
def _run_attn(q_nope, q_pe, ckv_cache, kpe_cache, sparse_indices, sm_scale):
...
def custom_kernel(data: input_t) -> output_t:
    """Compute fused sparse index + attention."""
    (
        q_index_fp8,
        k_index_cache_fp8,
        weights,
    ...
    # Run indexer
    topk_indices = _run_index(q_index_fp8, k_index_cache_fp8, weights, seq_lens, block_table)[0]
    sparse_indices = topk_indices.to(torch.int32)

    # Run attention
    return _run_attn(q_nope, q_pe, ckv_cache, kpe_cache, sparse_indices, sm_scale)
