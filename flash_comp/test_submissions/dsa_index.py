...
TORCH_LIBRARY(dsa_index_module, m) {
    m.def(
        "dsa_topk_indexer_launch("
        "Tensor q_index_fp8, "
        "Tensor k_index_cache_fp8, "
        "Tensor weights, "
        "Tensor seq_lens, "
        "Tensor block_table, "
        "Tensor spill_scores, "
    ...
    m.impl("dsa_topk_indexer_launch", &dsa_topk_indexer_launch);
}
"""


load_inline(
    name="dsa_topk_indexer_ops",
    cpp_sources="",
...
dsa_topk_indexer_launch = torch.ops.dsa_index_module.dsa_topk_indexer_launch
...
def _get_topk_output(device: torch.device, batch: int, topk: int) -> torch.Tensor:
    dev_key = (device.type, -1 if device.index is None else device.index)
    out = _topk_output.get(dev_key)

    if out is None or out.shape[0] < batch or out.shape[1] < topk:
        rows = batch if out is None else max(batch, out.shape[0])
        cols = topk if out is None else max(topk, out.shape[1])
        out = torch.empty((rows, cols), dtype=torch.int32, device=device)
        _topk_output[dev_key] = out

    return out[:batch, :topk]
...
def custom_kernel(data: input_t) -> output_t:
    ...
    batch = int(q_index_fp8.shape[0])
    topk_indices = _get_topk_output(q_index_fp8.device, batch, 2048)
