...
def run(input, weight, eps, residual=None):
    """
    Reference implementation of RMSNorm with hidden_size=128.

    Args:
        input: Input tensor of shape (B, 128) in bfloat16
        weight: Weight tensor of shape (128,) in bfloat16
        eps: Small epsilon value for numerical stability
        residual: Optional residual tensor of shape (B, 128) in bfloat16
...
def test_correctness(batch_size=8, with_residual=True, atol=8e-3, rtol=1e-2):
    ...
    print(f"\n{'='*60}")
    print(f"Testing RMSNorm h128: batch_size={batch_size}, with_residual={with_residual}")
    print(f"{'='*60}")
...
def main():
    ...
    print("Testing RMSNorm h128 Reference Implementation")
