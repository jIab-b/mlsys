...
# 2CTA cluster kernel (bench1: G=8, K=7168, 176 tiles).
cuda_src_2cta = """
...
# 2CTA cluster kernel with per-cluster stages (bench2: G=8, K=2048, 364 tiles).
cuda_src_stages = """
#include <cudaTypedefs.h>
#include <cuda_fp16.h>
#include <cuda_fp8.h>
...
struct KernelParams {
    ...
    uint16_t lut_worker_start[MAX_CLUSTERS];
    uint16_t lut_worker_count[MAX_CLUSTERS];
    uint16_t lut_worker_main_stg[MAX_CLUSTERS];
    uint8_t lut_worker_ns[MAX_CLUSTERS];
    uint8_t _pad_worker_ns[MAX_CLUSTERS];
...
inline void build_tile_lut(KernelParams& params, int schedule) {
    ...
    for (int cid = num_clusters; cid < MAX_CLUSTERS; cid++) {
        params.lut_worker_start[cid] = 0;
        params.lut_worker_count[cid] = 0;
        params.lut_worker_main_stg[cid] = 0;
        params.lut_worker_ns[cid] = 0;
        params._pad_worker_ns[cid] = 0;
    }
...
void grouped_gemm_kernel(
    ...
    const int NS = static_cast<int>(params.lut_worker_ns[cluster_id]);
    const int main_stg = static_cast<int>(params.lut_worker_main_stg[cluster_id]);
    const int smem_sf = smem_main + NS * main_stg;
...
void grouped_gemm_impl(
    ...
    int raw_total_tiles = 0;
    int group_b_bytes[MAX_GROUPS][3] = {};
    for (int g = 0; g < G; g++) {
        int Mi = A_list[g].size(0);
        int Ki = A_list[g].size(1) * 2;
        int Ni = B_list[g].size(0);
        ...
        group_b_bytes[g][0] = B_HALF;
        if (m_tail == 0) {
            group_b_bytes[g][1] = B_HALF;
            group_b_bytes[g][2] = B_HALF;
        } else {
            group_b_bytes[g][1] = b_tail0_box_h * BLOCK_K / 2;
            const int b_tail1_box_safe = b_tail1_box_h > 0 ? b_tail1_box_h : 1;
            group_b_bytes[g][2] = b_tail1_box_safe * BLOCK_K / 2;
        }
        raw_total_tiles += mt * nt;
    }
    ...
    // Per-cluster stage depth: derive max B payload from tiles assigned to each cluster.
    int max_cluster_ns = 1;
    int max_cluster_main_stg = A_SIZE + 1 * BLOCK_K / 2;
    for (int cid = 0; cid < num_clusters; cid++) {
        const int worker_start = static_cast<int>(params.lut_worker_start[cid]);
        const int my_count = static_cast<int>(params.lut_worker_count[cid]);
        TORCH_CHECK(my_count > 0, "worker must own at least one tile");

        int max_b_bytes = 1 * BLOCK_K / 2;
        for (int t = 0; t < my_count; t++) {
            const int lut_idx = worker_start + t;
            const uint32_t meta0 = params.lut_meta0[lut_idx];
            const uint32_t meta1 = params.lut_meta1[lut_idx];
            const int gidx = static_cast<int>(meta0 & 0x7U);
            const int b_tmap_r0 = static_cast<int>((meta1 >> 6U) & 0x3U);
            const int b_tmap_r1 = static_cast<int>((meta1 >> 8U) & 0x3U);
            TORCH_CHECK(b_tmap_r0 >= 0 && b_tmap_r0 <= 2, "b_tmap_r0 out of range");
            TORCH_CHECK(b_tmap_r1 >= 0 && b_tmap_r1 <= 2, "b_tmap_r1 out of range");

            const int b0 = group_b_bytes[gidx][b_tmap_r0];
            const int b1 = group_b_bytes[gidx][b_tmap_r1];
            if (b0 > max_b_bytes) max_b_bytes = b0;
            if (b1 > max_b_bytes) max_b_bytes = b1;
        }

        const int main_stg_c = A_SIZE + max_b_bytes;
        int best_ns_c = 1;
        for (int ns = MAX_NS; ns >= 1; ns--) {
            int total_smem = ns * main_stg_c + ns * SF_STAGE;
            if (total_smem <= smem_avail) {
                best_ns_c = ns;
                break;
            }
        ...
        params.lut_worker_main_stg[cid] = static_cast<uint16_t>(main_stg_c);
        params.lut_worker_ns[cid] = static_cast<uint8_t>(best_ns_c);
        if (main_stg_c > max_cluster_main_stg) max_cluster_main_stg = main_stg_c;
        if (best_ns_c > max_cluster_ns) max_cluster_ns = best_ns_c;
    }

    // Keep legacy scalar fields as launch-wide maxima for debug visibility.
    params.main_stg = max_cluster_main_stg;
    params.ns = max_cluster_ns;
...
TORCH_LIBRARY(gg_cg2_cta_noep_stages, m) {
    m.def("run(Tensor[] A, Tensor[] B, Tensor[] C, Tensor[] SFA, Tensor[] SFB) -> ()");
    m.impl("run", &grouped_gemm_impl);
}
"""

# Non-transposed CG1 pairmax kernel (bench3/4: G=2).
cuda_src_pairmax = """
#include <cudaTypedefs.h>
#include <cuda_fp16.h>
#include <cuda_fp8.h>
...
//   [3:0]    mma_m_q16
//   [4]      a_tmap
//   [13:5]   expect_q128 (9 bits, max 511)
struct KernelParams {
    GroupInfo groups[MAX_GROUPS];
    int num_groups;
    int total_tiles;
...
// ============================================================================
__host__ __device__ inline uint32_t pack_tile_meta0(
    uint32_t gidx, uint32_t coord_m, uint32_t coord_n, uint32_t tile_num_k
) {
    return (gidx & 0x7U)
         | ((coord_m & 0x3FFU) << 3U)
         | ((coord_n & 0x3FFU) << 13U)
...
__host__ __device__ inline uint32_t pack_tile_meta1(
    uint32_t mma_m_q16, uint32_t a_tmap, uint32_t expect_q128
) {
    return (mma_m_q16 & 0xFU)
         | ((a_tmap & 0x1U) << 4U)
         | ((expect_q128 & 0x1FFU) << 5U);
}
...
__device__ inline void decode_tile_runtime(
    ...
    uint32_t meta0 = params.lut_meta0[lut_idx];
    uint32_t meta1 = params.lut_meta1[lut_idx];
    t.gidx = static_cast<int>(meta0 & 0x7U);
    t.coord_m = static_cast<int>((meta0 >> 3U) & 0x3FFU);
    t.coord_n = static_cast<int>((meta0 >> 13U) & 0x3FFU);
    t.tile_num_k = static_cast<int>((meta0 >> 23U) & 0xFFU);
    t.mma_m = static_cast<int>((meta1 & 0xFU) << 4U);
    t.a_tmap = static_cast<int>((meta1 >> 4U) & 0x1U);
    t.expect_bytes = static_cast<int>(((meta1 >> 5U) & 0x1FFU) << 7U);
    t.off_m = t.coord_m * BLOCK_M;
    t.off_n = t.coord_n * BLOCK_N;
}
    ...
    for (int g = 0; g < params.num_groups; g++) {
        ...
        for (int cn = 0; cn < n_tiles; cn++) {
            for (int cm = 0; cm < m_tiles; cm++) {
                ...
                TORCH_CHECK(g <= 7, "gidx overflow in packed tile meta");
                TORCH_CHECK(cm <= 1023, "coord_m overflow in packed tile meta");
                TORCH_CHECK(cn <= 1023, "coord_n overflow in packed tile meta");

                const bool is_m_tail = (cm == m_tiles - 1) && (mma_m_tail != BLOCK_M);
                const int mma_m = is_m_tail ? mma_m_tail : BLOCK_M;
                TORCH_CHECK((mma_m & 15) == 0, "mma_m must be multiple of 16");
                TORCH_CHECK(mma_m >= 16 && mma_m <= 128, "mma_m out of supported range");

                int a_tmap = is_m_tail ? 1 : 0;
                int a_bytes = is_m_tail ? (m_rem * BLOCK_K / 2) : A_SIZE;
                int b_bytes = B_SIZE;
                int expect = a_bytes + b_bytes + SFA_SIZE + SFB_SIZE;
                TORCH_CHECK((expect & 127) == 0, "expect must be 128-byte aligned");
                int expect_q128 = expect >> 7;
                TORCH_CHECK(expect_q128 <= 511, "expect_q128 overflow");

                tmp[total++] = {
                    pack_tile_meta0(
                        static_cast<uint32_t>(g), static_cast<uint32_t>(cm),
                        static_cast<uint32_t>(cn), static_cast<uint32_t>(tile_num_k)),
                    pack_tile_meta1(
                        static_cast<uint32_t>(mma_m >> 4),
                        static_cast<uint32_t>(a_tmap),
                        static_cast<uint32_t>(expect_q128)),
                };
            }
        }
    }
...
void grouped_gemm_kernel(
    ...
    if (warp_id == NUM_WARPS - 2 && elect_sync()) {
        ...
        for (int tile = 0; tile < my_count; tile++) {
            ...
            const int tile_num_k = tile_rt.tile_num_k;
            const int coord_m = tile_rt.coord_m;
            const int coord_n = tile_rt.coord_n;
            const int off_m = tile_rt.off_m;
            const int off_n = tile_rt.off_n;
            const int tma_expect_bytes = tile_rt.expect_bytes;

            const void* A_tmap = get_a_tmap(gidx, tile_rt.a_tmap);
...
// Host launch
// ============================================================================
void grouped_gemm_impl_pairmax(
    at::TensorList A_list,
    at::TensorList B_list,
    at::TensorList C_list,
    at::TensorList SFA_list,
...
TORCH_LIBRARY(gg_cg1_noep_notranspose, m) {
    m.def("run(Tensor[] A, Tensor[] B, Tensor[] C, Tensor[] SFA, Tensor[] SFB) -> ()");
    m.impl("run", &grouped_gemm_impl_pairmax);
}
"""
...
load_inline(
    "grouped_gemm_cg2_cta_noep_stages",
    cpp_sources="",
    cuda_sources=cuda_src_stages,
    is_python_module=False,
    no_implicit_headers=True,
    extra_cuda_cflags=_cuda_cflags,
    extra_ldflags=["-lcuda"],
...
load_inline(
    "grouped_gemm_cg1_noep_notranspose",
    cpp_sources="",
    cuda_sources=cuda_src_pairmax,
    is_python_module=False,
    no_implicit_headers=True,
    extra_cuda_cflags=_cuda_cflags,
    extra_ldflags=["-lcuda"],
...
_run_stages = torch.ops.gg_cg2_cta_noep_stages.run
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if num_groups == 8:
        # CG2 tile counts: 256-wide N-pair tiles
        total_tiles = 0
        for i in range(num_groups):
            ...
            total_tiles += mt * nt
        if total_tiles == 364:
            _run_stages(list(a), list(b), list(c), list(sfa), list(sfb))
        else:
            _run_2cta(list(a), list(b), list(c), list(sfa), list(sfb))
    else:
        _run_pairmax(list(a), list(b), list(c), list(sfa), list(sfb))
