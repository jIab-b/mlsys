...
template <int32_t M, int32_t N, int32_t K, int32_t BLOCK_M, int32_t BLOCK_N,
          int32_t BLOCK_K, int32_t PIPE_DEPTH, bool FAST_SILU, bool FAST_SILU2,
          bool TRANSPOSE, int32_t NUM_BLOCKS>
__global__ void __launch_bounds__(WARPGROUP_SIZE + 2 * WARP_SIZE, 1)
    nvfp4_dual_gemm_cutlass(__grid_constant__ const CUtensorMap a_map,
  ...
  constexpr int32_t num_rows = CEIL_DIV(M, BLOCK_M);
  constexpr int32_t num_cols = CEIL_DIV(N, BLOCK_N);
  ...
  if (warp_id == 4 && elect_one_sync()) {
    ...
    fence_barrier_init();
  } else if (warp_id == 5 && elect_one_sync()) {
    prefetch_tma_descriptor(&a_map);
    prefetch_tma_descriptor(&b1_map);
    prefetch_tma_descriptor(&b2_map);
  } else if (warp_id == 0) {
      ...
      for (int32_t r0 = 0; r0 < 2; r0++) {
        for (int32_t r1 = 0; r1 < 2; r1++) {
          ...
          for (int32_t i = 0; i < (BLOCK_N / 4); i += 4) {
            ...
            float val1 = tmp[i] * acc2[i];
            float val2 = tmp[i + 1] * acc2[i + 1];
            float val3 = tmp[i + 2] * acc2[i + 2];
            float val4 = tmp[i + 3] * acc2[i + 3];
...
template <int32_t M, int32_t N, int32_t K, int32_t BLOCK_M, int32_t BLOCK_N,
          int32_t BLOCK_K, int32_t PIPE_DEPTH, bool FAST_SILU, bool FAST_SILU2,
          bool TRANSPOSE, int32_t GRID_DIM>
void launch_nvfp4_dual_gemm(uint8_t *A_ptr, uint8_t *B1_ptr, uint8_t *B2_ptr,
                            const uint8_t *SFA_ptr, const uint8_t *SFB1_ptr,
                            const uint8_t *SFB2_ptr, half *C_ptr) {
  ...
  constexpr cuuint64_t globalDimA[2] = {K, M};
  constexpr cuuint64_t globalStridesA[1] = {K / 2};
  constexpr cuuint32_t boxDimA[2] = {BLOCK_K, BLOCK_M};
  constexpr cuuint32_t elementStridesA[2] = {1, 1};
  ...
  constexpr cuuint64_t globalDimB[2] = {K, N};
  constexpr cuuint64_t globalStridesB[1] = {K / 2};
  constexpr cuuint32_t boxDimB[2] = {BLOCK_K, BLOCK_N};
  constexpr cuuint32_t elementStridesB[2] = {1, 1};
  CUDA_CHECK(cuTensorMapEncodeTiled(
  ...
  cudaFuncSetAttribute(
      nvfp4_dual_gemm_cutlass<M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, PIPE_DEPTH,
                              FAST_SILU, FAST_SILU2, TRANSPOSE, GRID_DIM>,
      cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_size);

  nvfp4_dual_gemm_cutlass<M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, PIPE_DEPTH,
                          FAST_SILU, FAST_SILU2, TRANSPOSE, GRID_DIM>
      <<<GRID_DIM, BLOCK_DIM, shmem_size>>>(tensorMapA, tensorMapB1,
                                            tensorMapB2, SFA_ptr, SFB1_ptr,
                                            SFB2_ptr, C_ptr);
}

#define LAUNCH(M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, PIPE_DEPTH, FAST_SILU,      \
               FAST_SILU2, TRANSPOSE, GRID_DIM)                                \
  launch_nvfp4_dual_gemm<M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, PIPE_DEPTH,       \
                         FAST_SILU, FAST_SILU2, TRANSPOSE, GRID_DIM>(          \
...
cuda_nvfp4_dual_gemm(const torch::Tensor &A, const torch::Tensor &B1,
  ...
  [[likely]] case hash(256, 4096, 7168):
    LAUNCH(256, 4096, 7168, 128, 64, 256, 5, true, false, false, 128);
    return C;
  [[likely]] case hash(512, 4096, 7168):
    LAUNCH(512, 4096, 7168, 128, 128, 256, 4, false, true, false, 128);
    return C;
  [[likely]] case hash(256, 3072, 4096):
    LAUNCH(256, 3072, 4096, 128, 64, 256, 5, true, false, false, 96);
    return C;
  [[likely]] case hash(512, 3072, 7168):
    LAUNCH(512, 3072, 7168, 128, 128, 256, 4, true, false, true, 96);
    return C.view({N, M, 1}).transpose(0, 1);
  [[unlikely]] case hash(1536, 512, 7168):
    LAUNCH(1536, 512, 7168, 128, 128, 256, 4, false, false, false, 48);
    return C;
  [[unlikely]] case hash(256, 512, 256):
    LAUNCH(256, 512, 256, 128, 128, 256, 4, false, false, false, 8);
    return C;
  [[unlikely]] case hash(3072, 1024, 1536):
    LAUNCH(3072, 1024, 1536, 128, 128, 256, 4, false, false, false, 192);
    return C;
  [[unlikely]] case hash(7168, 1024, 256):
    LAUNCH(7168, 1024, 256, 128, 128, 256, 4, false, false, false, 448);
    return C;
  [[unlikely]] case hash(7168, 2304, 2048):
    LAUNCH(7168, 2304, 2048, 128, 128, 256, 4, false, false, false, 1008);
    return C;
  [[unlikely]] case hash(4608, 384, 7168):
    LAUNCH(4608, 384, 7168, 128, 128, 256, 4, false, false, false, 108);
    return C;
  [[unlikely]] case hash(7168, 384, 2304):
    LAUNCH(7168, 384, 2304, 128, 128, 256, 4, false, false, false, 168);
    return C;
  [[unlikely]] case hash(512, 768, 7168):
    LAUNCH(512, 768, 7168, 128, 128, 256, 4, false, false, false, 24);
    return C;
  [[unlikely]] case hash(4096, 768, 512):
    LAUNCH(4096, 768, 512, 128, 128, 256, 4, false, false, false, 192);
    return C;
  [[unlikely]] default:
    return C;
  }
...
def custom_kernel(data: input_t) -> output_t:
    return my_module.cuda_nvfp4_dual_gemm(
        data[0], data[1], data[2], data[6], data[7], data[8], data[9]
    )
